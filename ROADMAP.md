...
| M1 Evidence architecture | Next | One bounded evidence model supplies a compact bootstrap and built-in read-only MCP. | Machine-readable OCR capabilities and current context contracts. | Implementation is active on issue #30: model, immutable snapshots, typed collectors/deltas, compact bootstrap, built-in read-only MCP, GitLab outcome rendering, history-backed parity and legacy removal, public-invocation synthetic E2E, two OCR review loops, the Codex Security correction loop, final Python 3.12/package gate, and signed local ready-PR checkpoint are complete; push/PR creation await authorization, then merge and deferred v0.4.0 release gates remain. |

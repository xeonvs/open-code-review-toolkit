...
flowchart LR
    ...
    M1 --> M5["M5 Bounded review-context enrichment<br/>in progress"]
    M3 --> M5
    M4 --> M5
...
| M5 Bounded review-context enrichment | In progress | Extend invocation evidence with bounded forge discussions and optional external records through one provider-neutral, capability-constrained context lifecycle, without a second review engine. | Established M1, M3, and M4 boundaries; frozen OCR/context contracts and active BL-023 delivery. | The v0.7.0 implementation acquires bounded snapshots, applies immutable protected-target context/DLP policy, authorizes every object before opaque handle minting, projects fixed `context_list`/`context_get` tools through the existing toolkit MCP, contains OCR persistence, validates publication independently, exposes partial/degraded outcomes, and blocks automatic approval for admitted mutable context. Stable product publication, independent artifact/provenance readback, and issue closure remain the establishment signal. |
...
- M5 is in progress through the complete local BL-023 implementation for v0.7.0; stable publication and external readback remain pending. Direct provider-specific MCP enforcement is not a second M5 path: external records stay behind the broker, while generic direct MCP retains the established M3 operator boundary. Its dependency graph is exactly `M1 -> M5`, `M3 -> M5`, and `M4 -> M5`; it is not a gate for M6 or M7. The architecture/threat checkpoint remains historical no-release evidence, while runtime implementation closes only through the stable release lifecycle.

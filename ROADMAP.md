...
flowchart LR
    ...
    M1 --> M5["M5 Bounded review-context enrichment<br/>in progress / planned"]
    M3 --> M5
    M4 --> M5
...
- M5 is in progress through the bounded 0.6.3 context/approval foundation in #100, while the complete BL-023 broker remains planned. Direct provider-specific MCP enforcement is not a second M5 path: external records stay behind the broker, while generic direct MCP retains the established M3 operator boundary. Its dependency graph is exactly `M1 -> M5`, `M3 -> M5`, and `M4 -> M5`; it is not a gate for M6 or M7. Its architecture/threat checkpoint is no-release, while runtime implementation closes through one stable release.

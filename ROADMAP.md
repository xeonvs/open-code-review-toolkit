...
- M5 is in progress through the bounded 0.6.3 context/approval foundation in #100, while the complete BL-023 broker and upstream-dependent BL-024 remain planned. Its dependency graph is exactly `M1 -> M5`, `M3 -> M5`, and `M4 -> M5`; it is not a gate for M6 or M7. Its architecture/threat checkpoint is no-release, while runtime implementation closes through one stable release.

...
| M1 Evidence architecture | Next | One bounded evidence model supplies a compact bootstrap and built-in read-only MCP. | Machine-readable OCR capabilities and current context contracts. | Implementation is active on issue #30: model, immutable snapshots, typed collectors/deltas, compact bootstrap, built-in read-only MCP, GitLab outcome rendering, history-backed parity and legacy removal, and the public-invocation synthetic E2E are complete; the final OCR/security correction loops, ready PR, merge, and deferred v0.4.0 release gates remain. |

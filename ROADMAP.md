...
| M1 Evidence architecture | Next | One bounded evidence model supplies a compact bootstrap and built-in read-only MCP. | Machine-readable OCR capabilities and current context contracts. | Implementation is active on issue #30: model, immutable snapshots, typed collectors/deltas, compact bootstrap, built-in read-only MCP, GitLab outcome rendering, and semantic parity are implemented; legacy removal, final E2E/OCR review, ready PR, and v0.4.0 publication gates remain. |

...
OCR receives every MCP as an independent named entry in its `mcp_servers` registry. The toolkit always installs `ocr_toolkit_evidence`; each configured local or remote MCP is an optional sibling started independently by OCR. Omitting `type` selects stdio. Stdio accepts `command`, `args`, literal `env`, `env_from`, `tools`, and `setup`; remote accepts an absolute HTTPS `url`, non-secret `headers`, secret `headers_from`, `tools`, and `setup`. Every optional server requires a non-empty explicit tool-name allowlist. Reserved names and cross-server tool collisions fail closed, so an external server cannot shadow mandatory evidence. `headers_from` writes a `$VARIABLE` reference and OCR expands it at connection time; sensitive literal headers are rejected.

```json
{
  "bounded_reference": {
    "command": "synthetic-read-only-proxy",
    "args": ["--fixed-corpus"],

...
| Context admission, degradation, mutation, and evidence use | Toolkit broker/store and fixed MCP receipts | Count-only receipt-v6 context/evidence state | Raw merge-request title, description, discussions, provider identities, rejected text, and record contents do not enter the receipt or telemetry event. |

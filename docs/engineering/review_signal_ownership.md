...
| Semantic grouping, group file membership, per-group spans, and filter activity | OCR runtime and OCR telemetry | Additive private result fields may be DLP-sanitized; no group or round field enters receipt v9, GitLab text, fingerprints, severity, lifecycle commands, toolkit telemetry, or approval | Group labels are model-produced. Group keys are sorted changed paths, so both are untrusted and potentially high-cardinality. |
| Tool requests and MCP use | OCR result for aggregate attempts; internal evidence and federation gateway for authenticated attempts/outcomes | Mandatory action-receipt-v3 accounting plus content-free per-alias federation reconciliation in receipt v9 | Arguments, paths, queries, scopes, IDs, results, headers and content are excluded. |
...
| Context admission, degradation, mutation, and evidence use | Toolkit broker/store and fixed MCP receipts | Count-only receipt-v9 context/evidence state | Raw merge-request title, description, discussions, CI provider identities/payloads, rejected text, and record contents do not enter the receipt or telemetry event. |
| Publication DLP and posting transaction state | Toolkit result projection and GitLab transaction owner | Receipt-v9 stage-aware state, one parseable summary marker, and one local count-only log event | OCR coverage, DLP omission and posting outcomes remain separate; source-class counts can overlap and no rejected value or location is emitted. |

...
| Tool requests and MCP use | OCR result for aggregate tool calls; each toolkit MCP owner for verified local use | Bounded known-server counts and mandatory evidence-action attribution in receipt v5 and the summary | Tool names and counts are closed; arguments, paths, IDs, results, headers, and content are excluded. |
...
| Context admission, degradation, mutation, and evidence use | Toolkit broker/store and fixed MCP receipts | Count-only receipt-v5 context/evidence state | Raw merge-request title, description, discussions, provider identities, rejected text, and record contents do not enter the receipt or telemetry event. |

...
## Planned bounded review-context enrichment

M5 is planned and disabled; no current release reads discussion comments, recognizes references, stores external records in a context broker, or provides `context_list`/`context_get`. When activated through BL-023, it extends bounded invocation evidence without reopening M1/M4 or creating a second review engine.

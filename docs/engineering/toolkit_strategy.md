...
Toolkit 0.6.3 activates only the context-selection, receipt, approval-identity, and GitLab-MR transport foundation tracked by #100. It does not activate discussion/reference acquisition, the broker/store/handle lifecycle, or enriched mode. Current OCR cannot enforce external-MCP annotation policy on the same registration session, so BL-024 remains blocked on upstream #997; until that follow-up is qualified, configured external MCP makes a review comment-only rather than carrying an enforceable read-only approval claim.

...
M5 is in progress at its bounded review-context selection and approval foundation; the complete enrichment lifecycle remains planned and disabled; no current release reads discussion comments, recognizes references, stores external records in a context broker, or provides `context_list`/`context_get`. When the complete lifecycle is activated through BL-023, it extends bounded invocation evidence without reopening M1/M4 or creating a second review engine.
...
During OCR, the model may list or read only minted handles through fixed toolkit-authored closed-schema tools projected by the existing toolkit MCP process. The M5 model loop has no upstream search, arbitrary URL/ID fetch, external MCP schema, redirect, traversal, write, or external network path. Context budgets cannot evict repository evidence. Forge authors use provider-declared account classes and run-local pseudonyms rather than names, email, avatars, or profile URLs. Unknown identity, authorization, DLP, completeness, version, or OCR capability fails closed; unavailable, partial, stale, or mutated context stays visible and cannot prove absence. Any admitted mutable discussion or external context makes automatic approval ineligible. It cannot change policy, tools, permissions, lifecycle commands, suppression, posting authority, or approval.

OCR remains the only model review engine. If a separate contextual adjudication phase is required, M5 waits for a native structured OCR capability instead of running and merging a second toolkit-driven review. OCR uses an isolated owner-only home with deterministic session cleanup; inability to contain or clean sessions blocks publication unless an explicit secure-debug mode was agreed before the run. Publication validation and DLP are independent from retrieval and model egress and cannot reverse an earlier disclosure to the model.

Toolkit 0.6.3 activates only the context-selection, receipt, approval-identity, and GitLab-MR transport foundation tracked by #100. It does not activate discussion/reference acquisition, the broker/store/handle lifecycle, or enriched mode. Configured direct external MCP makes a review comment-only rather than carrying an enforceable read-only approval claim. Same-session annotation enforcement is not retained as a separate M5 work item: annotations remain server-authored claims, and BL-023 places future external records behind toolkit-owned broker authorization and fixed tools instead of direct provider-specific model capabilities.

Public qualification remains synthetic. Issue-tracker, documentation/wiki, native API, and arbitrary MCP adapters are validated only behind the broker with dedicated credentials, installed artifacts, real OCR, protocol and persistence evidence, hostile providers, package/release gates, and explicit model-dependent claim limits.

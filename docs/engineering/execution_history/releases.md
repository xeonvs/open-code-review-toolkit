...
- **Feature branch:** `codex/v0.11.1-install-qualification`; PR #216 merged as `0b589f06ccd3df17ff6e9436ded88821038b4580`
...
Feature PR #216 passed exact-head CI, merged, and development TestPyPI workflow
[35831251267](https://github.com/xeonvs/open-code-review-toolkit/actions/runs/35831251267)
completed. Hosted OCR qualification workflow
[35831453766](https://github.com/xeonvs/open-code-review-toolkit/actions/runs/35831453766)
proved the adjacent 1.12.8–1.12.9 deterministic compatibility chain. Promotion
PR #219 passed exact-head CI and merged. Its development TestPyPI workflow
[35833829968](https://github.com/xeonvs/open-code-review-toolkit/actions/runs/35833829968)
completed, including registry verification.

This release PR sets stable 0.11.1, next development 0.12.0, source epoch
`1790149820`, and the exact sorted three-issue authorization set; renders the
Towncrier fragments, updates the public example to toolkit 0.11.1 and OCR
1.12.9, and leaves `.release-reconciled-version=0.11.0` pending independent
external readback. Stable registries, provenance, annotated tag, immutable
...
2. Done locally: qualification environment, safe failure status including setup
   failure, forward-only contracts and frozen historical evidence.
...
#### Engineering Workflow Upgrade 0.9.8

Status: completed; delivered through protected feature PR #216
Owner: root
...
- **Tracked issues:** #188–#193, #201–#207, #209 and #210 in milestone v0.11.0
- **Feature branch:** `codex/v0.11.0-mcp-federation`; PR #208 merged
...
- Feature PR #208 reviewed head `1e3b7d668874b2e86b2700284675d997a1654c0b`
  passed protected checks and squash-merged as
  `cc784c157d1b9efb3e45158e0c1a75fbb4801bd5`. Its complete diff self-review,
  final OCR qualification and Codex Security review were completed before merge.
- Local OCR v1.12.7 with model `openai/gpt-5.6-terra` reviewed the immutable
  `458d96711aec8f0379327f10cd03ccf3f3da180d...e5057d5417d6674f293b77636c49797e25ed14e4`
  range: 61/61 reviews, 22 mandatory evidence-MCP calls and four findings fixed
  with focused regressions. Private intermediate diagnostics remain owner-only.
- Codex Security scan `cc04b394-e16f-48b4-92ac-71cb2a27c963` completed the same
  immutable range with complete coverage and zero reportable findings.
- The first post-feature TestPyPI run 35442945566 published `0.11.0.dev97` but
  exposed a verifier defect: registry smokes installed immutable distributions
  without their runtime dependency closure. Corrective PR #211 added one
  hash-locked runtime-requirements artifact to both development and stable flows,
  passed all exact-head checks, and squash-merged as
  `3f402948ce4606b935587af63920c467af43fa3b`.
- Development run
  [35444762966](https://github.com/xeonvs/open-code-review-toolkit/actions/runs/35444762966)
  then published and fully verified `0.11.0.dev98`, including provenance, exact
  registry bytes, locked runtime dependency installation, and clean wheel/sdist
  CLI smokes. Wheel SHA-256 is
  `379684bce1983cb4058e1916bb4ae75c61d00cb8d7b0ccf45240b36b57122952`; sdist
  SHA-256 is `36236b7246c722f8df35495775ce6b80c38b8c76fe40e4e7aca22a68fee11d34`.
- This release PR sets `.release-version=0.11.0`, `.next-version=0.12.0`, source
  epoch `1789823158`, and the exact sorted 15-issue authorization set; renders
...
- **Feature branch:** `codex/v0.10.0-local-review`; feature PR #184 merged
- **Release branch:** `release/v0.10.0`
- **Repository completion date:** 2026-09-09
...
Replacement Draft #184 was reviewed at exact head
`d805df19a4f1e593a223b5a172b651c28209a734`, with all required hosted checks
successful and no unresolved review threads. It was squash-merged through the
protected workflow as `9cfa6b645119ed9230e790cca585376dd906dacc`; that
merge has reviewed tree `87edf15155692cdf60a03b13bd7e2daad7a996ce` and
sole parent `8ae890b6f78388554281e649dc8201737679968a`.

The corresponding main-push TestPyPI development workflow
[34345119338](https://github.com/xeonvs/open-code-review-toolkit/actions/runs/34345119338)
completed successfully: deterministic build, TestPyPI publication, provenance
verification, and immutable wheel/sdist installation smoke all passed. It is a
development build only, not stable-release delivery.

The feature PR description accidentally closed #181; it was reopened before
release preparation so both authorized release issues #181 and #182 are open
and valid for the Release workflow's pre-publication state check. This is a
lifecycle correction, not a claim of stable delivery.
...
nonblocking-pipe, blocking-pipe, full-pipe, and documentation tests passed
...
Towncrier 0.10.0 draft also passed. A fresh narrow security diff review covered
the signed remediation commit before it was published. Raw OCR diagnostics,
provider values, and private artifact locations remain unpublished.

#### Draft Readiness Receipt

Final feature head `d805df19a4f1e593a223b5a172b651c28209a734` passed every
required hosted check. CI run `34344557793`, build run `34344557755`, security
run `34344557775`, CodeQL run `34344557776`, and dependency-review run
`34344557756` all succeeded; the signed remediation commit was included in the
reviewed head. Draft #184 was then marked ready and merged through the protected
workflow as `9cfa6b645119ed9230e790cca585376dd906dacc`. Stable publication and
release lifecycle closure remain independent post-feature gates.
...
The signed `release/v0.10.0` branch starts at protected merge
`9cfa6b645119ed9230e790cca585376dd906dacc`. It sets
`.release-version=0.10.0`, `.next-version=0.10.1`, deterministic source epoch
`1788952887` (one second after the protected feature merge), and exact sorted
release issues `[181,182]`. It renders the Towncrier section, updates the
published GitLab example's stable toolkit pin, archives this complete plan, and
returns `PLANS.md` to its inactive template. `.release-reconciled-version`
intentionally remains `0.9.1` until the independent external readback is
complete.

Release preparation passed an ordinary complete self-review, the complete
quality gate (1,712 tests, 408 subtests, 86.72% coverage), focused release and
documentation contracts, Ruff formatting, lock and OCR-manifest validation,
pip-audit, release-note extraction, and `git diff --check`. Pinned Gitleaks
8.24.3 passed for the complete feature history. Two independent source-epoch
controlled builds are byte-identical and pass Twine: wheel SHA-256
...
Clean wheel and sdist installation with `ocr-ci --help` smoke passed. The prior
one authorized local OCR review is not repeated for release metadata and
documentation preparation.

The release PR must be titled exactly `Release v0.10.0`. It must state only
repository-side preparation. Stable TestPyPI/PyPI publication, provenance,
...
| WQ-06 | done | Final reconciliation commit, exact-head push and hosted checks |
...
- [x] Code, tests, public docs, issues and feature PR agree.
- [x] Local/remote head and hosted feature checks agree.
...
- Feature PR #178 reviewed exact head `b47fa551b5de42dcfe9cd5b316a32f4184525be6` with all 13 hosted checks successful and zero unresolved review threads, then squash-merged as `de9055e699ff8a7bc0806c6eae5bdf6666e95160`; the remote and local feature branches are absent.
...
Deliver a backward-compatible, privacy-bounded remediation-thread context source, exact GitLab mention commands, a complete environment contract, mode-oriented GitLab examples, Accepted project decisions usage guidance, and navigation indexes. Integrate OCR 1.9.10 as the exact toolkit 0.8.0 target, raise meaningful boundary coverage, and take the exact final head through protected release delivery after the owner explicitly waived the remaining enriched-context qualification without treating that missing receipt as completed evidence.
...
- Finish each corrective cycle with a locally self-reviewed signed head and exact-head hosted CI. Retain the completed direct OCR 1.9.10 review as private remediation input; do not run the separately planned enriched-context qualification after the owner's explicit waiver, and do not represent its missing receipt as accepted evidence.
...
- Do not modify or disclose owner-only OCR credentials or endpoint configuration, the user's unrelated `HOME` content, or start a local model peer. No further local OCR or qualification run is authorized after the owner's waiver.
- Do not run a separate Codex Security scan or disable Bandit/Gitleaks/CodeQL. Do not merge, publish stable 0.8.0, or close release issues/milestone before the independent protected lifecycle gates that own those transitions.
...
- Do not install OCR 1.9.9. Its accepted evidence remains historical; only OCR 1.9.10 is the current integration target.
...
- The separate enriched real-path qualification is waived by the owner; its receipt is absent and must not be claimed. Stable publication remains governed by the protected feature/release PR, registry, provenance, artifact, and external-readback gates.
...
- Waived external qualification: no enriched receipt exists for OCR 1.9.10 `context_list`/`context_get`, still-present/evidence-resolved scenarios, or receipt-level raw-data leakage inspection. This is an explicit owner waiver, not a successful qualification or a replacement for the completed deterministic tests, direct OCR remediation review, hosted checks, package gates, or stable-release external readback.
...
- Risk: a corrective push is treated as accepted before hosted checks bind to that exact head. Recovery: record the exact commit/tree, require all hosted checks again, and keep the PR Draft until exact-head checks and review threads are accepted.
- Risk: the waived enriched qualification leaves no external receipt for model use of `context_list`/`context_get` or its receipt-level leakage audit. Recovery: retain the waiver in the archived plan and release coordination, never claim that evidence, and reopen a separately authorized qualification or corrective release if later production evidence invalidates the deterministic contracts.
...
- Risk: OCR version prose conflates inherited 1.9.9 evidence with the 1.9.10 deployment target. Recovery: keep separate headings in compatibility docs and the OCR maintenance fragment and assert exact manifest/preflight/example defaults.
...
Open the exact `Release v0.8.0` PR from `release/v0.8.0`, require exact-head hosted checks and resolved review threads, then use the protected squash merge to start stable publication. Independently verify registry/workflow/Release bytes, provenance, attestations, installs, annotated tag, immutable receipt, issue receipts, issue and milestone closure, synchronized `main`, and final scratch cleanup. Do not run another local OCR qualification or claim the absent enriched receipt.
...
- [x] The work queue preserves every logical slice and records the external qualification as explicitly waived rather than completed.
...
- [x] All in-scope `REQ-###` items are `done`; `WQ-11` records the owner waiver and does not claim a qualification receipt.
...
- [x] The final local validation matrix and hosted CI are green for exact reviewed feature head `e2c5b847a3884edeeeb24123412af51b280faf14`; protected merge `ff540214a93e184ccba11f4f11f1b0153619bc57` preserves its reviewed tree.
- [x] `PLANS.md`, roadmap/strategy, generated Towncrier changelog, issues, milestone, and remote refs agree that implementation is complete and protected release delivery is active with the enriched receipt explicitly waived.
- [x] Plan lifecycle validation succeeds for the repository-complete release checkpoint.

### Post-Close Delivery

- Feature PR #122 and its exact-tree protected merge are complete. TestPyPI development run 32724113797 and independent bytes/provenance/install readback are complete.
- The user waived the remaining enriched qualification on `2026-08-24`; no receipt is accepted or claimed. Release-PR merge, stable publication, immutable external readback, and issue/milestone closure remain separately gated lifecycle states.
...
- Do not start another local OCR or enriched qualification run. Keep owner-only OCR configuration, private results, and endpoint/model/token details out of tracked history and public release surfaces.
- Keep OCR 1.9.9 inherited and OCR 1.9.10 changed/deployed text separate in evidence, docs, changelog, PR, and #120.
...
- Feature PR [#122](https://github.com/xeonvs/open-code-review-toolkit/pull/122) passed all checks at reviewed head `e2c5b847a3884edeeeb24123412af51b280faf14` and was squash-merged as `ff540214a93e184ccba11f4f11f1b0153619bc57`; the merge tree equals reviewed feature tree `cec4be33c6fff27de93fded041cde06a798d1ca7`.
- TestPyPI development run [32724113797](https://github.com/xeonvs/open-code-review-toolkit/actions/runs/32724113797) published `0.7.2.dev66`. Independent readback matched the workflow wheel SHA-256 `9f13dabc12860442722f1109b8eeeb85804901ddcad5e3d74a5096b9d4b67a34` and sdist SHA-256 `1d4dc6dfc9b4b4a311eb6043a5b671103122bc7e6b8ebaae85ddc466ded3f07f`, verified the exact PEP 740 publisher and subjects, and clean-installed both artifacts with CLI smoke.
- The release PR is the final repository mutation. It sets `.release-version=0.8.0`, `.next-version=0.8.1`, deterministic source epoch `1787572363` (one second after the feature squash merge), exact sorted issues `[120, 123, 124, 125, 126, 127]`, generated Towncrier notes, stable example pins, and reconciled roadmap/strategy/documentation truth; `PLANS.md` is reset.
- Release-candidate validation passes 95 focused release/documentation tests and the canonical 1,185-test plus 277-subtest quality gate at 85.91% branch coverage, with locked risk groups at 83%, 81%, 85%, and 87%; Ruff, strict MyPy, Bandit, lock, OCR-manifest, dependency-audit, release-note, Twine, archive-content/privacy, and diff checks pass. Two deterministic 0.8.0 builds are byte-identical with wheel SHA-256 `1b7caba9bc74ee7643ee344bb5bec949a1f02fe1a7f9189de08f971a409dd620` and sdist SHA-256 `9757cc4f5677e6f366458c6248cf72df8979072ed69264022224998e786725c4`; clean wheel and sdist install plus CLI smoke pass on Python 3.12, 3.13, and 3.14.
- The owner's waiver remains explicit: no enriched OCR+LLM receipt proves model-time `context_list`/`context_get`, still-present/evidence-resolved scenarios, or receipt-level raw-data leakage inspection. Stable publication and external readback do not replace that absent qualification.
- Stable TestPyPI/PyPI bytes, GitHub attestations, registry provenance, annotated `v0.8.0` tag, immutable GitHub Release and assets, `release-receipt.json`, supported-Python registry installs, Actions-owned issue receipts, issue and milestone closure, and final clean-main synchronization are not claimed by this repository checkpoint and remain post-merge external gates.

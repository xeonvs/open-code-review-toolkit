...
Publication validation runs after OCR and before result acceptance. It compares whole forbidden/non-publishable values and display-normalized contiguous excerpts of at least 24 characters, including closed HTML entity/comment/tag and Markdown escape/formatting forms; smaller values remain protected as whole values. It separately checks configured secret values, closed PII patterns, controls, Markdown destinations, and Unicode deception. A comparison that would exceed the fixed work bound is uncertainty and blocks ordinary comments, as does any projection-state uncertainty or match; only a static toolkit-authored failure remains publishable. This is containment of deterministic exact-output classes, not a claim to detect shorter arbitrary excerpts, arbitrary semantic paraphrase, or undo content already sent to the model.

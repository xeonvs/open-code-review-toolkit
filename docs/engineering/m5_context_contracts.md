...
Automatic approval preserves every existing manifest, coverage, warning, omission, finding, exact-SHA, author, provider, and self-approval gate. Required-source degradation and any admitted mutable generic discussion, remediation thread, or external record block approval. Optional degradation is visible and cannot prove source absence. DLP-clean title, description, generic discussion, and external context with zero admitted mutable records do not block approval solely because they were inspected. DLP rejection degrades its source and can never restore or enable approval. Remediation admission is an independent comment-only condition regardless of the reply's wording.

...
Publication validation runs after OCR and cleanup, within the same inode-checked atomic read/replace that attaches receipt v4. It compares whole forbidden/non-publishable values and display-normalized contiguous excerpts of at least 24 characters, including closed HTML entity/comment/tag and Markdown escape/formatting forms; smaller values remain protected as whole values. It separately checks configured secret values, closed PII patterns, controls, Markdown destinations, and Unicode deception. A comparison that would exceed the fixed work bound is uncertainty and filters the affected publication unit. Unsafe finding content omits the finding; unsafe optional finding fields are removed independently. The raw result is atomically replaced by a safe partial result containing independently passed findings/warnings and closed reason, omission, original-coverage, and tool-use facts. GitLab may publish that safe subset with an explicit filtered/partial signal, while retaining the previous review, suppressing duplicates already visible there, and blocking approval. Unsafe values/locations and a raw debug copy are not retained. This is containment of deterministic exact-output classes, not a claim to detect shorter arbitrary excerpts, arbitrary semantic paraphrase, or undo content already sent to the model.

...
- **Activation trigger:** Not met. OCR already exposes explicit per-run provider/model overrides and an independent aggregate budget. Activate only after repeated operations demonstrate that direct settings are insufficient and the owner approves a closed model/provider matrix plus precedence contract.
- **Upstream overlap:** OCR 1.8.7 satisfies the direct provider/model capability dependency. OCR 1.9.0's per-file ceiling and OCR 1.9.5's aggregate review budget can reduce coverage, so they are not profile defaults: operators must select them explicitly and partial coverage remains visible. Deterministic tool rendering and cache optimizations likewise need no toolkit alias.
- **Goal:** If demonstrated need appears, offer `economy`, `standard`, and `strong` aliases for one OCR run without hiding review-completeness controls.
- **Scoped deliverables:** Define an owner-approved closed provider/model matrix; map one profile to OCR's per-run overrides rather than mutating persistent OCR configuration; publish the effective profile and observed additive result identity without credentials; validate availability through compatibility capabilities, environment precedence, and rendered effective configuration.
...
- **Exclusions:** Aggregate token budgets or other hidden coverage limits inside profiles, per-file/per-tool model routing, hidden heuristics, multi-agent orchestration, or full-repository scan profiles.
...
- **Priority:** medium
- **Roadmap theme:** M5 Review profiles and quality measurement
- **Dependencies:** Established discussion/fingerprint lifecycle, structured OCR result normalization, review-health reporting, failed-file coverage, finding/posting receipts, and MCP-use attribution. BL-016 is relevant only if later comparisons use named aliases; it is not required for the gap audit.
...
- **Upstream overlap:** OCR 1.8.10's deterministic tool rendering and OCR 1.9.1's Anthropic cache optimization reduce upstream comparison or execution cost noise. OCR 1.9.4's terminal session ID is useful high-cardinality correlation evidence. OCR 1.9.5 adds `summary.budget_exceeded` to full-repository `scan`, which the toolkit does not invoke; its diff `review` already supplies token totals, budget warnings, completed findings, and budget-attributed partial manifest coverage. These remain OCR-owned signals, not a reason to add scan support or duplicate telemetry. The now-ready audit must determine whether current upstream review telemetry and toolkit result-derived receipts already suffice.

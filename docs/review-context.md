...
The toolkit uses no shell or setup hook. It starts the exact executable with a clean allowlisted environment and an isolated owner-only working directory/home, exchanges one bounded JSONL request and response, enforces a deadline, terminates the process group on failure, and retains only private bounded redacted stderr while the run is active.
...
After OCR exits, the toolkit independently scans the entire result against forbidden/non-publishable context, configured secrets, closed PII patterns, controls, Markdown destinations, and Unicode/HTML/Markdown laundering forms. It compares whole values and normalized contiguous excerpts of at least 24 characters; a work-bound uncertainty also filters the affected publication unit. This does not claim detection of shorter arbitrary excerpts or semantic paraphrases, and it cannot reverse data already sent to the model.

A filtered result is neither retained raw nor discarded wholesale. In the same inode-checked atomic transformation used to attach the receipt, the toolkit replaces it with a safe partial result: independently DLP-passed finding/warning fields, fixed tool-use counters, original closed coverage counts, and closed DLP reason/retained/omitted counts. Unsafe `content` removes its finding; an unsafe optional finding field is removed without discarding safe finding content. Unsafe values and their locations are never retained. GitLab may publish the safe subset, but marks it partial, preserves the previous review, suppresses duplicates already visible there, and cannot automatically approve it.

The summary contains a dedicated `Publication filtering signal` details block and an exact `ocr.publication-dlp-signal/v1` HTML marker with only low-cardinality counts. The posting command emits the same JSON as an `OCR toolkit telemetry event` log line so CI log collectors can alert without a new runtime network/exporter path. OCR remains authoritative for provider/token/request telemetry; v0.7.0 does not add an OTLP client, arbitrary telemetry endpoint, raw value/path, or mandatory external export. There is no raw-result or secure-debug retention switch.

Receipt v4 stores only closed review/policy identities, context mode, per-source completeness and degradation counts, admitted-mutable state, fixed evidence/context tool-use counts, publication-DLP result, and cleanup result. It does not store context text, provider IDs, URLs, commands, arguments, headers, adapter results, personal display data, or transport diagnostics. Receipt v1-v3 remains readable for existing comments but cannot authorize current automatic approval.

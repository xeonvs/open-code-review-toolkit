...
Each OCR version owns one stable HTML marker and one qualification issue. The workflow performs a single upsert through bounded direct issue listing rather than GitHub's eventually consistent search index. If historical duplicates are present it fails closed instead of creating another issue; operators select and reconcile the canonical issue once, after which reruns update that issue in place.

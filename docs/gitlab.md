...
Install `open-code-review-toolkit` from PyPI. Install upstream Open Code Review separately and pin `v1.7.11`; verify the release checksum before making the binary executable. The package never downloads OCR.
...
- `OCR_LLM_TOKEN`: the LLM gateway credential used by upstream OCR.
- `OCR_SHA256`: the trusted checksum for the pinned upstream binary asset.
...
`ocr-ci preflight` validates the installed OCR version, GitLab access, and configured LLM model. `configure` and `mcp-config` write upstream OCR configuration without invoking a config subprocess. `context` creates bounded Markdown. `post` interprets a JSON artifact and publishes bounded notes with rollback and ownership safeguards.

Upstream OCR is configured through its `openai-responses` provider. Optional stdio bridge tools are supplied with `OCR_MCP_SERVERS_JSON`; treat every configured MCP command as privileged code.

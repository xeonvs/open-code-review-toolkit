...
The pipeline explicitly sets `OCR_GITLAB_TARGET_PROTECTION_MODE: "required"`. This is the secure default: an unprotected target is rejected before OCR or model execution, and an explicit empty, malformed, mixed-case, or unknown value also fails closed. Set exact `unprotected` only when the project deliberately permits unprotected targets. That setting does not constrain a target GitLab reports as protected; normal protected behavior remains available. When the actual target is unprotected, receipt v7 enforces limited, comment-only review regardless of `OCR_AUTO_APPROVE`.

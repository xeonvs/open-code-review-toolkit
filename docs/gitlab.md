...
- governed federation credentials named by reviewed `token_from`, `headers_from`, or local-only `env_from` entries.
...
The pipeline explicitly sets `OCR_GITLAB_TARGET_PROTECTION_MODE: "required"`. This is the secure default: an unprotected target is rejected before OCR or model execution, and an explicit empty, malformed, mixed-case, or unknown value also fails closed. Set exact `unprotected` only when the project deliberately permits unprotected targets. That setting does not constrain a target GitLab reports as protected; normal protected behavior remains available. When the actual target is unprotected, receipt v9 enforces limited, comment-only review regardless of `OCR_AUTO_APPROVE`.
...
`OCR_REVIEW_CONTEXT_MODE` selects `off`, `metadata`, or `enriched`; governed federation is configured independently through registry v2.
...
| [Unprotected target](../examples/gitlab/modes/unprotected-target.gitlab-ci.yml) | `off` or bounded `metadata`; exact-target Rules are untrusted guidance | Built-in immutable evidence only; no protected policy or federation | Structurally comment-only when the actual target is unprotected |
...
| [Governed MCP federation](../examples/gitlab/modes/direct-mcp.gitlab-ci.yml) | Metadata plus model-selected admitted tool results | Registry-v2 HTTPS upstream through the bounded gateway | Used advisory tools are comment-only; unused services and successful `review_read` use are not independent blockers |

The [bounded-context recipes](../examples/gitlab/context/) implement the
[protected policy v4](review-context.md#protected-target-policy-v4) contract for
discussions, remediation history, and exact same-revision CI outcomes. Copy one template to `.opencodereview/review-context-policy.json` and merge it into the protected target branch before selecting `enriched`; a source-branch copy cannot expand authority. Missing or invalid policy stops before OCR. Legacy v1-v3 policies remain parse-only: safe selectors continue, optional references are skipped, and required references make the run comment-only. Configure external tools separately with registry v2. DLP rejection and required-source degradation block approval; every admitted remediation thread does too.

Governed federation is operator-owned and GitLab accepts only registry-v2 HTTPS upstreams. The gateway freezes the allowlist, validates schemas, arguments, origins, DLP and bounds, and reconciles content-free outcomes. Use dedicated least-privilege credentials and service-side tenant/object/field/operation authorization; gateway controls do not make an overprivileged service identity safe.

For an actually unprotected target, only `off` and bounded untrusted `metadata` context are supported. `enriched` and external federation are rejected before OCR rather than silently downgraded. The mandatory toolkit-owned repository-evidence MCP remains available because it reads immutable Git objects and has no provider network or arbitrary identifier path. The configured repository Rules file is required and loaded only from the exact captured target SHA as bounded untrusted model guidance. Accepted decisions and structured target guidance are omitted; Rules cannot authorize tools, acquisition, suppression, posting, or approval.

The normal review outcome remains authoritative. A valid unprotected receipt adds the static line *The target branch was not protected in GitLab. This review ran in limited, comment-only mode.* after the primary status line without turning complete coverage into partial coverage. Legacy, malformed, or contradictory receipts cannot produce that trusted limitation state.

## First integration sequence

Use two merge requests for the recommended setup:

1. merge `examples/gitlab/rules.json` or the chosen repository Rules path and any target-owned review configuration into the target branch;
2. enable the OCR CI job in a later merge request, after the captured target commit contains that configuration.

If review of the integration merge request itself is unnecessary, adding Rules and the CI job together is supported under the default protected-target path. The first run stops before OCR and posts the recognized setup-pending note only after the closed pre-execution status matches the effective source and diff-base identities. Retrying that same merge request cannot make source-branch Rules become target-owned, so it does not help; subsequent merge requests work after the setup change merges. Missing, malformed, stale, or mismatched status identity uses the generic fail-closed failure note instead of promising a detailed public diagnostic.

An exact merge-request lifecycle read also precedes evidence collection. If the identity-bound MR is already `merged` or `closed`, no OCR/model work starts and one toolkit-owned plain status note is created or updated without replacing findings, summaries, suppression, discussions, or approval state. Every path that could otherwise return advisory success confirms the exact MR is still terminal; close/reopen timing therefore fails and requires a new review even after a note-side error. Its marker is recognized only in a reserved control preamble, never from finding text. `merged` is irreversible and may finish successfully; `closed` stays advisory-only, while `OCR_STRICT_POSTING=true` remains nonzero so a required gate cannot be reused after reopening. A result admitted while the MR was open performs the same exact read immediately before its first publication mutation: terminal results retain ordinary findings/DLP/limits/transaction behavior, name the terminal state and reviewed commit, and skip approval. Unknown states, provider failures, and project/IID/SHA mismatches fail closed before publication.

GitLab Code Owners and code-owner approval rules are separate project merge policy. They are not prerequisites for OCR execution and cannot give unprotected repository content policy authority. Likewise, a green advisory pipeline or allowed-to-fail OCR job proves neither that `ocr-ci review` completed nor that OCR or its model executed. Inspect the review result, manifest coverage, receipt, and published status.
...
`ocr-ci preflight` validates OCR compatibility, GitLab access, registry v2, upstream inventory, and optional model metadata. `ocr-ci configure` writes the isolated OCR configuration. `ocr-ci review` captures the exact source head and target SHA plus the actual protection state, collects immutable repository evidence, acquires only context permitted by that state, applies DLP and budgets, runs OCR once under an owner-only isolated home, reconciles federation, validates the result, cleans private state, and attaches receipt v9. `ocr-ci post` hostile-reads that receipt rather than reconstructing configuration from a later environment.
...
`enriched` loads the protected policy and may add one stable GitLab discussion snapshot, verified toolkit-owned remediation threads, and exact same-revision CI outcomes. Generic discussion and remediation projections are mutually exclusive for a toolkit-owned root. Remediation roots require both the authenticated live bot ID and a valid toolkit marker/fingerprint. Raw GitLab IDs, usernames, provider objects, and rejected values are not stored or returned. The model sees only opaque local handles; it cannot search GitLab or submit an arbitrary provider ID or URL.

`OCR_DLP_ENABLED` defaults to enabled. A parsed false value disables context/federation and publication DLP, emits a prominent GitLab warning, and makes the run comment-only. Service egress and GitLab publication may then expose sensitive data, and the toolkit cannot retract it. Diagnose locally with `--preserve-private-artifacts` before considering this escape hatch; invalid values fail before OCR.
...
When a merge request introduces a repository-owned OCR rules path absent from both trusted protected baselines, `review` stops before OCR and `post` may publish only the static setup-pending message after hostile identity validation. The source file never becomes policy evidence for its own merge request. Under an actually unprotected target, target Rules are still mandatory but remain untrusted guidance; a missing target blob fails before OCR without promoting the source copy.

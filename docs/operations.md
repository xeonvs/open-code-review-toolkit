...
Publication DLP admits horizontal tab only in the `existing_code` and
`suggestion_code` fields; all other control/format characters remain blocking.
When the public projection is incomplete, posting preserves prior review state
and reports publication filtering separately from the original OCR coverage.
...
Publication reporting keeps three stages separate: OCR manifest coverage,
publication-DLP projection, and GitLab posting outcomes. A complete OCR run stays
complete when DLP omits a finding or field. Posting counts describe admitted
findings; DLP omission totals and their bounded overlapping source-class counts
are shown separately. Previous comments are preserved with the actual incomplete
stage reason instead of being replaced by a falsely clean review.

`OCR_DLP_ENABLED` defaults to strict `true`. Exact `false` disables context,
federation and publication DLP for that run, emits a prominent warning in the
summary and Technical details, and makes automatic approval unreachable. It can
expose sensitive content through service/model egress or GitLab publication, and
the toolkit cannot retract prior egress or writes. Diagnose locally with
`--preserve-private-artifacts` before using it. Invalid or empty values fail before
OCR; non-DLP bounds, identity, evidence, cleanup and posting transaction gates
remain active.
...
When a fully validated receipt v9 records an actually unprotected target, the summary adds exactly *The target branch was not protected in GitLab. This review ran in limited, comment-only mode.* immediately after the primary status line. It appears for clean, findings, warnings, partial, budget-stopped, failed-result, and publication-filtered summaries without replacing or reclassifying the canonical outcome. The limitation is a trust and approval boundary, not a coverage result. A protected target does not show it merely because the operator allowed unprotected targets; legacy, malformed, or contradictory receipt state cannot show it as trusted status.

## Automatic approval lifecycle

`OCR_AUTO_APPROVE=true` is the default. Approval is a separate transaction only
after every current review note publishes. A review is eligible only with exact closed receipt v9 for a protected target, enabled DLP, a supported complete manifest, no warnings, failures, waivers, token-budget stop, or omitted findings, no used advisory federation tool or uncertain federation accounting, no degraded metadata, no DLP-rejected selected source, no required context degradation, no admitted remediation context, and at most three findings. Receipt v1-v8 is rejected by posting and approval. Receipt v9 binds reviewed source, policy, and target SHA, actual `protected|unprotected` target state, merge-request author ID, context mode/state, DLP mode, per-source completeness/degradation, admitted-mutable state, fixed context-tool use, mandatory evidence state, content-free federation reconciliation, stage-aware publication DLP, and cleanup. An actually unprotected target is structurally ineligible: orchestration does not call the approval executor, regardless of `OCR_AUTO_APPROVE`. Unused federation services and successful `review_read` use do not independently block approval. `private-sanitized` remains eligible only when its canonical publication/approval projection is byte-equivalent; `publication-filtered` is partial and ineligible. Complete `metadata` context, complete non-remediation enrichment, and the built-in evidence/context MCP do not independently block approval on a protected target. Every finding must have
...
preserve valid review signal; genuine DLP failures in public fields still filter
those fields and leave a safe summary. Raw OCR stderr remains private. See the
[diagnostic decision flow](review-decision-flow.md#additive-failed-tool-diagnostic-states).

Receipt v9 is emitted only for a validated GitLab merge-request profile with a real `protected|unprotected` target observation. An ordinary local review still receives the same result parsing, evidence/federation reconciliation, and publication DLP, but remains receipt-less because local Git state cannot establish GitLab protection. The compatible receipt-less path remains approval-ineligible and cannot render receipt-derived federation or target-protection claims; any present malformed or incomplete receipt fails closed before prior review state is read.

Run OCR through `ocr-ci review --result PATH --stderr PATH -- ...`. The wrapper owns preview, one OCR model loop, mandatory evidence, governed federation lifecycle, result validation, stage-aware DLP, cleanup and receipt v9. It does not post to GitLab. A recognized soft background diagnostic is attached after publication DLP and rendered only with an exact receipt; hard rejection stops before the model. Cleanup or federation-receipt uncertainty blocks normal publication. DLP-filtered findings and fields are omitted without relabelling original OCR coverage; previous comments remain and the `ocr.publication-dlp-signal/v3` marker reports only closed omission and source-class counts. Posting never reconstructs or republishes rejected values.

OCR may add group labels, file membership, grouping-task records, round diagnostics, readable reasoning, and provider-native replay payloads to private session data. They remain outside the canonical finding/posting projection, receipt v9, DLP counts, toolkit telemetry, tool/token summary, and approval inputs. If they appear inside receipt v9, validation fails closed. Caller `--output`/`-o` is rejected before preview because the wrapper owns the result descriptor and posting handoff.
...
The review step emits at most one numeric line beginning `OCR provider diagnostics:`. It distinguishes HTTP `402` as `http-payment-required` and `429` as `http-rate-limited`, includes a shared bounded status only when every terminal failure has that status, and includes only non-zero failed, cancelled, retried, total-retry, and recovered request counters. Mixed terminal details are sorted and count-aggregated rather than emitted per request. These protocol facts do not prove ordinary throttling, a spending policy, or cost reservation from the requested output cap. The line is local operator feedback, not toolkit telemetry, a receipt field, a DLP input, a GitLab summary field, or an approval signal.
...
Raw provider/model identities, response bodies, error codes and messages, request IDs, paths, warnings, and stderr remain in owner-only private artifacts for a classified provider failure. `OCR_POST_ERROR_DETAILS=1` cannot add them to that note or the numeric line. Normal findings from the failed result are ignored, the previous successful review is preserved, and automatic approval is not attempted. Missing, oversized, malformed, or internally contradictory retry reports keep the existing generic failure path instead of guessing a classification.

For a local diagnosis, add `--preserve-private-artifacts` before the `--` separator. The command retains owner-only private review material and deliberately leaves no posting-eligible receipt. Inspect it locally before considering `OCR_DLP_ENABLED=false`, keep it out of commits and shared artifacts, and remove it afterward. The escape hatch can expose sensitive content through service/model egress, local Markdown or GitLab and cannot retract prior disclosure.

Three pre-execution outcomes have narrower static reporting. When the merge request introduces the configured repository-owned OCR rules path and that exact path is absent from both immutable policy-side baselines, `review` verifies only that the source object is a bounded regular blob, writes a closed private status, and stops before OCR. When installed OCR rejects the generated background during preview, the status instead carries the closed character/file-size reason plus actual, limit, and unit. When the exact identity-bound GitLab merge request is already `merged` or `closed`, the toolkit stops before evidence collection and OCR, then upserts one toolkit-owned plain status note. `post` hostile-validates the v3 status against the current source and diff-base identities and, for the terminal variant, the exact provider/project/MR target. Every non-strict success path re-reads that exact lifecycle before returning; a reopened, unavailable, unknown, or mismatched MR fails and requires a new review even when note collection or verification also failed. A `merged` MR is irreversible and may return success after exact note readback. A `closed` MR remains reopenable: advisory mode may report the expected skip successfully, but `OCR_STRICT_POSTING=true` keeps the required gate nonzero so reopening cannot reuse a green no-review result. The terminal note states that no model ran, no findings were produced, approval was not attempted, and prior review/reviewer state remains intact. Duplicate jobs update the same note; an ambiguous or multiply matched note fails closed. The terminal marker is control metadata only in its reserved preamble line, so marker-like finding text cannot select, overwrite, or delete a review note. A successful ordinary result removes a stale terminal note only after publication succeeds, including safe partial publication.

For a receipt-v9 result that was already admitted before the merge request became terminal, `post` re-reads the exact project, MR IID, state, and reviewed source SHA immediately before the publication transaction. `opened` keeps ordinary behavior. `merged` or `closed` keeps the normal findings, summary, DLP, posting limit, partial-publication, rollback, and reconciliation contracts, adds the terminal state and reviewed commit to Technical details, and makes automatic approval explicitly skipped. Malformed, unknown, unavailable, or identity-mismatched lifecycle data stops before the first publication write. Once publication has started, existing transaction and rollback behavior remains authoritative.

A successful retry replaces only an earlier toolkit-owned setup-pending note; background rejection and generic failures never replace previous review findings or summaries. Static outcomes include no rules/background path or raw stderr and trust no repository/provider display text. Malformed, stale, unsafe, unknown, or identity-mismatched state falls back to the generic failure note. `OCR_POST_EMOJI=false` removes the heading emoji; `OCR_POST_ERROR_DETAILS` does not add detail to recognized static outcomes.

The setup-pending case is the supported one-merge-request installation path, not a completed review. Retrying the same merge request does not help because the captured target commit still lacks target-owned Rules; merge the setup change and use a later merge request. In detached pipelines, the recognized note uses the same validated effective reviewed SHA as execution: a valid non-zero MR SHA takes precedence, while an absent or all-zero MR SHA may use valid `CI_COMMIT_SHA`. If neither identity is valid or the status does not match it, the generic fail-closed note is intentional. A green advisory job still does not prove OCR/model execution; require a valid result, complete manifest coverage where expected, reconciled receipt/action counts, and the published review state.

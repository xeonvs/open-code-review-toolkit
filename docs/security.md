...
- GitLab lifecycle decisions consume only a strict `opened|merged|closed` state after exact numeric project/MR identity and reviewed SHA validation. A terminal state before admission cannot reach evidence or OCR; a terminal transition after admission cannot authorize approval or bypass result validation, DLP, limits, or the posting transaction. Unknown, malformed, unavailable, or mismatched provider data fails closed before a publication mutation.

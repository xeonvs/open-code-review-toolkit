...
8. The complete OCR result crosses independent publication DLP, deterministic GitLab publication, suppression, and receipt-v5 approval policy.
...
- Automatic approval binds the exact synchronized reviewed head and MR author from receipt v5, skips self-approval, and never removes an existing approval. Partial, warning, non-v5, publication-filtered, omitted, degraded metadata, required context degradation, admitted mutable context, or direct external-MCP runs are ineligible.
...
- **Approval/suppression manipulation:** any admitted mutable discussion, remediation thread, or external context blocks automatic approval; partial or DLP-rejected context cannot prove absence or restore eligibility. Safe MR metadata or safe non-remediation context does not itself block an otherwise eligible receipt. Remediation text cannot change severity, prove a fix, suppress or resolve a finding, or issue a lifecycle command. Existing suppression, exact reviewer commands, fingerprints, and discussion ownership remain separate deterministic consumers.

...
def pypi_release_json_manifest(
    payload: dict[str, Any], version: str, expected_hashes: dict[str, str]
) -> list[dict[str, str]]:
    """Validate PyPI's version JSON as a v0.11.1-only index recovery source."""

    if version != "0.11.1":
        raise PreviewError("PyPI release JSON recovery is limited to v0.11.1")
    _validate_local_hashes(version, expected_hashes)
    info = payload.get("info")
    if not isinstance(info, dict) or info.get("name") != PACKAGE or info.get("version") != version:
        raise PreviewError("PyPI release JSON identity mismatch")
    urls = payload.get("urls")
    names = expected_filenames(version)
    if not isinstance(urls, list) or len(urls) != len(names):
        raise PreviewError("PyPI release JSON has an incomplete or extra artifact set")
    converted = []
    seen: set[str] = set()
    for item in urls:
        if not isinstance(item, dict):
            raise PreviewError("PyPI release JSON file entries must be objects")
        filename = item.get("filename")
        if not isinstance(filename, str) or filename not in names or filename in seen:
            raise PreviewError("PyPI release JSON has an unexpected or duplicate artifact")
        seen.add(filename)
        expected_type = "bdist_wheel" if names[filename] == "wheel" else "sdist"
        if item.get("packagetype") != expected_type or item.get("yanked") is not False:
            raise PreviewError(f"PyPI release JSON has invalid file state: {filename}")
        digests = item.get("digests")
        sha256 = digests.get("sha256") if isinstance(digests, dict) else None
        if sha256 != expected_hashes[filename]:
            raise PreviewError(f"PyPI release JSON SHA-256 mismatch: {filename}")
        url = item.get("url")
        try:
            has_query = isinstance(url, str) and bool(urlsplit(url).query)
        except ValueError as exc:
            raise PreviewError(
                f"PyPI release JSON has an invalid download URL: {filename}"
            ) from exc
        if not isinstance(url, str) or has_query:
            raise PreviewError(f"PyPI release JSON has an invalid download URL: {filename}")
        converted.append(
            {
                "filename": filename,
                "hashes": {"sha256": sha256},
                "url": url,
                "provenance": f"https://pypi.org/integrity/{PACKAGE}/{version}/{filename}/provenance",
            }
        )
    return artifact_manifest({"files": converted}, version, expected_hashes, PYPI_ARTIFACT_HOST)
...
def main() -> int:
    ...
    recovery_parser = subparsers.add_parser("pypi-release-json-manifest")
    recovery_parser.add_argument("--version", required=True)
    recovery_parser.add_argument("--index-json", required=True, type=Path)
    recovery_parser.add_argument("--hashes-json", required=True, type=Path)
    recovery_parser.add_argument("--artifact-host", choices=[PYPI_ARTIFACT_HOST], required=True)
    ...
    try:
        ...
        elif args.command == "pypi-release-json-manifest":
            payload = json.loads(args.index_json.read_text(encoding="utf-8"))
            if not isinstance(payload, dict):
                raise PreviewError("PyPI release JSON response must be an object")
            print(
                json.dumps(
                    pypi_release_json_manifest(
                        payload, args.version, load_hashes(args.hashes_json)
                    ),
                    sort_keys=True,
                )
            )

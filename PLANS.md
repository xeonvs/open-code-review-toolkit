...
5. [x] Complete typed dependency/runtime/container/guidance collection and deltas for both refs without routing facts through legacy Markdown (BL-005).
...
8. [x] Implement and register the bounded read-only `ocr_toolkit_evidence` stdio MCP server (BL-007).
...
- MCP checkpoint validation: 88 focused evidence, CLI, MCP-configuration, and runtime-helper tests plus 15 subtests pass; Ruff and strict mypy pass. The built-in server completes initialize, tools/list, summary, list, get, cursor binding, request/response bounds, and safe-error contracts over stdio, while generated private artifacts remain mode 0600 and existing parent-directory permissions are preserved. A real local stdio handshake against the repository evidence store returned the read-only `ocr_toolkit_evidence` tool and a bounded summary. OCR-driven non-zero tool-call verification remains part of the pre-removal E2E gate.
...
- Public integration migration in progress: the synthetic GitLab example now builds `.review-context/evidence.json` plus a compact bootstrap and exports the store path used by the built-in MCP server. User, configuration, security, and architecture docs describe the new read-only evidence path; the legacy command remains intentionally undocumented during the parity window and will be physically removed only after E2E.

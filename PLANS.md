...
- `WQ-17` (`in_progress`): raise the combined floor to 85%, add four locked risk-group Coverage.py reports to local and hosted quality execution, finish separately categorized OCR/coverage/runtime fragments and documentation, then run the complete local validation and holistic requirements/privacy/architecture/telemetry/data-flow review.
...
Continue at `WQ-17`: add only meaningful missing boundary tests needed to reach combined 85% and the remaining risk-group floors, wire the existing Coverage.py reports into local and hosted quality execution, finish the coverage maintenance fragment, then run the complete local validation and holistic review. Do not push until `WQ-18`.

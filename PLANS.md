...
5. [ ] Complete typed dependency/runtime/container/guidance collection and deltas for both refs without routing facts through legacy Markdown (BL-005). The typed-only path, source-aware identities, container images, guidance, Ansible Galaxy parity, Python, and JavaScript declarations/lockfiles are implemented and validated. The v0.4 package/runtime floor is Python 3.12, matching the documented `python:3.12-slim` CI integration and allowing the standard-library TOML parser to remain dependency-free, with tested support through Python 3.14. Missing lockfiles remain represented as absent resolved facts rather than an invented error: only a present candidate can be malformed or unavailable. Go, Composer/PHP, and the remaining application/infrastructure facts follow as separate signed checkpoints with focused/full tests, self-review and plan/backlog reconciliation.
...
8. [ ] Complete the bounded read-only `ocr_toolkit_evidence` stdio MCP integration (BL-007): toolkit-owned artifact discovery, executable-free OCR registration, review-lifecycle startup, bounded diagnostics, and proven non-zero calls from real OCR.

...
- [ ] **A. Architecture and OCR capability checkpoint.** Rebuild the live capability matrix; qualify OCR 1.9.7 and 1.9.8 with official digests and source review; promote 1.9.8; move `.next-version` to `0.7.0`; freeze exact policy, adapter, store, handle, receipt, containment, publication, and one-pass OCR contracts; assign production owners in the threat/evidence matrix.
- [ ] **B. Mandatory evidence bootstrap (#111).** Put a toolkit-authored `action=summary` requirement before repository-derived bootstrap data, retain zero-call rejection, and prove that preflight self-query does not count.

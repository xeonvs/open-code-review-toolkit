...
- `REQ-015` (`pending`): make the v0.8.0 release-note delta equally actionable for a production-integration agent and a human operator: categorize each outcome by effect, explicitly label additions, changes, removals, defaults, and migrations, and name exact public symbols and replacements. Record removed environment variables in a separate `maintenance` fragment. Covered by `WQ-07` and `WQ-09`.
...
Continue at `WQ-08`: reorganize GitLab examples around the mode matrix, move context recipes, demonstrate Accepted project decisions consumption, remove user-facing `synthetic` terminology and obsolete migration prose, pass the slice commit gate, and do not push.

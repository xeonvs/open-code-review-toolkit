...
- **Status:** active; planning and external decomposition complete, implementation not started.
...
- **Deferred existing-M5 work:** BL-023 retains the complete enriched context broker. BL-024 is [#103](https://github.com/xeonvs/open-code-review-toolkit/issues/103), blocked on upstream Open Code Review [#997](https://github.com/alibaba/open-code-review/issues/997). Neither is a `0.6.3` delivery claim.
...
The release explicitly does not claim enforceable external-MCP read-only semantics. OCR 1.9.5 has no same-session hook that can reject a discovered tool from its current annotations before registration/model projection. A reconnecting probe has a capability-swap race and is not a substitute. BL-024 owns future qualification after the upstream hook exists. Full `enriched` discussion/reference acquisition, protected context policy, broker/store/handles, DLP, session containment, and publication projections remain in BL-023.
...
7. Only after external readback, verify Actions-owned closure of #100/#101, close milestone `v0.6.3`, leave #103/BL-024 and upstream #997 open, retain unfinished BL-023, and reconcile clean local `main`.

#### Current checkpoint and exact resume action

Planning sources, GitHub milestone `v0.6.3`, #100/#101 assignment, BL-024 #103, and upstream OCR #997 are created and read back. No runtime, test, public contract, changelog, or release file has been changed. After this planning-only commit is signed, pushed, and represented by a draft PR, resume with slice 1 locally and do not push again until every complete-branch gate above passes.

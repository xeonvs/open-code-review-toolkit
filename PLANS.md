...
- Close the development-publication provenance gap found during independent readback before preparing the stable release: the canonical registry verifier must bind an explicit expected workflow, `testpypi.yml` for development and `release.yml` for stable publication, and the development verify job must validate PEP 740 publisher and exact subjects rather than only bytes and installs.
...
8. [x] Merge protected feature PR #94 after exact-head checks and independently verify TestPyPI `0.6.2.dev56` artifact hashes plus supported-Python installs from workflow `32009213205`.
9. [ ] Close the independently discovered development-provenance verification gap and the subsequent stale cross-version temp-directory collision through protected follow-up PRs, then verify exact TestPyPI development publisher, subjects, bytes, and installs.
10. [ ] Prepare and merge the protected `Release v0.6.2` PR as the final repository mutation, then independently verify stable registries, hashes, provenance/attestations, annotated tag, immutable GitHub Release, receipt, supported-Python installs, and issue closure.
...
- Feature PR #94 merged as `13093602a0b40521641447c9d31ed61754e90aea` with reviewed-tree equivalence. TestPyPI workflow `32009213205` published `0.6.2.dev56`; workflow-artifact and registry hashes agree and TestPyPI-only Python 3.12-3.14 installs pass. Independent PEP 740 readback exposed that the development publisher is correctly `testpypi.yml`, while the canonical verifier was hard-coded to `release.yml` and the development verify job checked bytes/installs only. The follow-up keeps one verifier owner and requires explicit workflow identity instead of weakening publisher validation.
- The follow-up routes development verification through the canonical bounded registry verifier, requires an allowlisted expected workflow identity at every caller, and retains `release.yml` for both stable registries. Focused 40-test contracts, the full quality gate, YAML/shell validation, and the unchanged production verifier against real `0.6.2.dev56` pass. The real end-to-end rerun caught and fixed an initially omitted shell-to-Python `--workflow` handoff that unit-level publisher tests alone did not prove.
- PR #95 merged as `cdab8671439cd94de476c1290113e835fce5dcf0`; hosted run `32010083361` verified `0.6.2.dev57` through the corrected publisher boundary. Independent dev57 readback confirmed bytes and provenance, then exposed that the helper reused one registry-wide `/tmp` artifact directory and its wheel glob admitted retained dev56 alongside dev57. The next narrow follow-up scopes artifact and venv paths by the already validated version so cross-version leftovers cannot enter the install proof.

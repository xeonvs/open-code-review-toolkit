...
- `WQ-15` (`in_progress`): add focused fault tests at existing private-result, parser, preflight, GitLab transport, posting transaction, rollback, strict/non-strict, and previous-review owners; review the complete `artifact -> parser -> posting -> GitLab` flow and make only contract-required production fixes.
...
Continue at `WQ-15`: add meaningful private-result, preflight, GitLab transport/transaction, rollback, strict/non-strict, and prior-review fault tests in their existing owners; fix production only for a demonstrated contract violation. Do not push until `WQ-18`.

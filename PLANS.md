...
5. [ ] Complete typed dependency/runtime/container/guidance collection and deltas for both refs without routing facts through legacy Markdown (BL-005). The typed-only path, source-aware identities, container images, guidance, and Ansible Galaxy parity are implemented; full Python/JavaScript/Go/Composer declaration, lock, runtime, absence, and diagnostic parity remain active.
